...
class QuantumCriticConfig:
    ...
    depolarising_error: bool = False
    gate_control_noise: bool = False
    noise_level: float = 0.01


def build_quantum_device(config: QuantumCriticConfig):
    use_noise = config.depolarising_error or config.gate_control_noise
    shots = config.shots

    if use_noise:
        return qml.device("default.mixed", wires=config.n_qubits, shots=shots)
...
class VQCCritic(nn.Module):
    def __init__(self, input_dim: int, output_dim: int = 1, qcfg: Optional[QuantumCriticConfig] = None):
        super().__init__()
        self.qcfg = qcfg or QuantumCriticConfig()
        self.n_qubits = self.qcfg.n_qubits
        self.n_layers = self.qcfg.n_layers
        self.ansatz_type = self.qcfg.ansatz_type
            ...
            for layer_index in range(self.n_layers):
                ...
                if self.ansatz_type == 1:
                    for qubit_index in range(self.n_qubits):
                        ...
                        qml.RZ(weights[layer_index, qubit_index + self.n_qubits], wires=qubit_index)
                    for qubit_index in range(self.n_qubits - 1):
                        qml.CZ(wires=[qubit_index, qubit_index + 1])
                else:
                    for qubit_index in range(self.n_qubits):
                        qml.CRY(weights[layer_index, qubit_index], wires=[qubit_index, (qubit_index + 1) % self.n_qubits])
...
class GaussianActor(nn.Module):
    def __init__(self, obs_dim: int, act_dim: int = 3, hidden_dims: Sequence[int] = (256, 128)):
        super().__init__()
        layers = []
        last_dim = obs_dim
        ...
        self.log_std = nn.Parameter(th.full((act_dim,), -1.0))
    ...
    def _squashed_log_prob(self, dist, pre_tanh_action: th.Tensor, squashed_action: th.Tensor) -> th.Tensor:
