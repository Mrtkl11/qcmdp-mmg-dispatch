...
class QCMDPConfig:
    ...
    n_qubits: int
    n_layers: int
    critic_encoder_hidden: int
    quantum_weight_scale: float
    shots: Optional[int]
    ...
    lambda_lr: float
    cost_scale: float
    cost_critic_coef: float
    reward_adv_scale: float
    cost_adv_scale: float
    dual_update_horizon: int
    target_kl: float
    log_interval: int
...
def compute_gae(
    ...
    for index in reversed(range(len(rewards))):
        ...
        td_error = (
            rewards[index] + gamma * next_values[index] * not_done - values[index]
        )
...
class QCMDPSingleMGTrainer:
    def __init__(
        self, env: IEEE33SingleMGEnv, config: Mapping[str, object] | QCMDPConfig
    ):
        self.env = env
        self.config = (
            config
            if isinstance(config, QCMDPConfig)
            else QCMDPConfig.from_mapping(config)
        )
        ...
        qcfg = QuantumCriticConfig(
            ...
            encoder_hidden=self.config.critic_encoder_hidden,
            weight_scale=self.config.quantum_weight_scale,
        ...
        self.reward_critic_opt = optim.Adam(
            self.reward_critic.parameters(), lr=self.config.critic_lr
        )
        self.safety_critic_opt = optim.Adam(
            self.safety_critic.parameters(), lr=self.config.critic_lr
        )
    ...
    def collect_rollout(self) -> Tuple[RolloutBatch, float, float]:
            ...
            with torch.no_grad():
                if done:
                    ...
                    next_reward_value = float(
                        self.reward_critic(next_obs_tensor).item()
                    )
                    next_safety_value = float(
                        self.safety_critic(next_obs_tensor).item()
                    )
        ...
        avg_reward = (
            ...
            if episode_rewards
            else float(cumulative_reward)
        )
        avg_cost = (
            float(np.mean(episode_costs)) if episode_costs else float(cumulative_cost)
        )
    ...
    def _update_critics(
        ...
        reward_loss = (
            0.5
            * (reward_prediction - mb_reward_returns).pow(2).mean()
            * self.config.vf_coef
        )
        self.reward_critic_opt.zero_grad()
        reward_loss.backward()
        torch.nn.utils.clip_grad_norm_(
            self.reward_critic.parameters(), self.config.max_grad_norm
        )
        ...
        safety_loss = (
            0.5
            * (safety_prediction - mb_cost_returns).pow(2).mean()
            * self.config.vf_coef
            * self.config.cost_critic_coef
        )
        self.safety_critic_opt.zero_grad()
        safety_loss.backward()
        torch.nn.utils.clip_grad_norm_(
            self.safety_critic.parameters(), self.config.max_grad_norm
        )
    ...
    def _update_actor(
        ...
        surrogate_2 = (
            torch.clamp(ratio, 1.0 - self.config.clip, 1.0 + self.config.clip) * mb_adv
        )
        actor_loss = (
            -torch.min(surrogate_1, surrogate_2).mean() - self.config.ent_coef * entropy
        )
    ...
    def update(self, rollout: RolloutBatch) -> UpdateStats:
        ...
        compound_advantages = normalize_vector(
            ...
            - float(self.lagrange_multiplier)
            * self.config.cost_adv_scale
            * cost_advantages
        )
        ...
        old_logp_tensor = torch.tensor(
            rollout.logp, dtype=torch.float32, device=self.device
        )
        reward_returns_tensor = torch.tensor(
            reward_returns, dtype=torch.float32, device=self.device
        )
        cost_returns_tensor = torch.tensor(
            cost_returns, dtype=torch.float32, device=self.device
        )
        advantages_tensor = torch.tensor(
            compound_advantages, dtype=torch.float32, device=self.device
        )
        ...
        for _ in range(self.config.epochs):
            ...
            for start in range(0, data_size, self.config.batch_size):
                ...
                self._update_critics(
                    obs_tensor,
                    reward_returns_tensor,
                    cost_returns_tensor,
                    batch_indices,
                )
                approx_kl = self._update_actor(
                    obs_tensor,
                    act_tensor,
                    old_logp_tensor,
                    advantages_tensor,
        ...
        mean_step_cost = (
            float(np.mean(rollout.real_cost)) if len(rollout.real_cost) > 0 else 0.0
        )
        horizon_cost = mean_step_cost * max(1, self.config.dual_update_horizon)
        dual_error = horizon_cost - self.config.cost_limit
        self.lagrange_multiplier = float(
            max(
                0.0,
                self.lagrange_multiplier + self.config.lambda_lr * dual_error,
            )
        )
    ...
    def train(self) -> List[TrainIterationStats]:
        ...
        for update_index in range(1, self.config.updates + 1):
            ...
            if (
                update_index % self.config.log_interval == 0
                or update_index == 1
                or update_index == self.config.updates
            ):
                print(
    ...
    def save_checkpoint(self, output_dir: str | Path) -> None:
        ...
        torch.save(
            {
                "lagrange_multiplier": bundle["lagrange_multiplier"],
...
def operation_history_to_dataframes(operation_history: List[Mapping[str, object]]):
    ...
    for _, row in history_df.iterrows():
        for bus_index, voltage_value in enumerate(
            row.get("bus_voltage_pu", []) or [], start=1
        ):
            voltage_rows.append(
                {
...
def plot_power_balance(history_df: pd.DataFrame, save_path: str, title: str):
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        pv_share = np.divide(
            pv_raw,
            renewable_total,
            out=np.zeros_like(pv_raw),
            where=renewable_total > 1e-9,
        )
        wind_share = np.divide(
            wind_raw,
            renewable_total,
            out=np.zeros_like(wind_raw),
            where=renewable_total > 1e-9,
        )
...
def plot_voltage_profile(
    ...
    if voltage_df.empty:
        return
    aggregated = (
        voltage_df.groupby("bus")["voltage_pu"]
        .agg(["min", "mean", "max"])
        .reset_index()
    )
...
def plot_network_power_metrics(metrics_df: pd.DataFrame, save_path: str):
    ...
    ax.plot(
        x,
        metrics_df["network_total_loss_kw"].astype(float).values,
        label="Network loss (kW)",
        color="tab:blue",
    )
    ax.set_xlabel("Time step")
    ...
    ax2.plot(
        x,
        metrics_df["slack_p_mw"].astype(float).values,
        label="Slack P (MW)",
        color="tab:orange",
    )
...
def plot_line_loading(line_df: pd.DataFrame, save_path: str, title: str):
    ...
    snapshot = (
        line_df[line_df["timestamp"] == latest_timestamp].copy().sort_values("branch")
    )
...
def plot_training_rewards(
    training_history: List[TrainIterationStats], save_path: str, ma_window: int
):
    ...
    x_ma = (
        np.arange(ma_window, len(episode_returns) + 1)
        if len(episode_returns) >= ma_window
        else x
    )
...
def save_training_history(
    training_history: List[TrainIterationStats], save_path: str | Path
) -> pd.DataFrame:
...
def run_evaluation_and_plot(
    ...
    print(
        f"[{output_dir}] running deterministic {env.episode_horizon}-step dispatch evaluation"
    )
    ...
    history_df, voltage_df, line_df, metrics_df = operation_history_to_dataframes(
        history
    )
    ...
    plot_power_balance(
        history_df,
        str(output_dir_path / "power_balance.png"),
        f"Power Balance ({mg_name})",
    )
    plot_voltage_profile(
        voltage_df,
        str(output_dir_path / "voltage_profile_summary.png"),
        f"Voltage Profile ({mg_name})",
        env.voltage_min_pu,
        env.voltage_max_pu,
    )
    plot_voltage_heatmap(
        voltage_df,
        str(output_dir_path / "voltage_heatmap.png"),
        f"Bus Voltage Heatmap ({mg_name})",
        env.voltage_min_pu,
        env.voltage_max_pu,
    )
    plot_network_power_metrics(
        metrics_df, str(output_dir_path / "network_power_metrics.png")
    )
    plot_line_loading(
        line_df,
        str(output_dir_path / "line_loading_last_step.png"),
        f"Line Loading Snapshot ({mg_name})",
    )
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Single-microgrid Q-CMDP training entrypoint."
    )
    parser.add_argument(
        "--data-path", type=str, default=None, help="Path to the external dataset CSV."
    )
    parser.add_argument(
        "--mg-id", type=int, default=1, help="Microgrid index in [1, 5]."
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default=None,
        help="Directory for weights, logs, and figures.",
    )
    parser.add_argument(
        "--config-json",
        type=str,
        ...
        help="JSON file containing the environment and Q-CMDP experiment configuration.",
    )
    parser.add_argument(
        "--updates", type=int, default=None, help="Optional update-count override."
    )
    parser.add_argument(
        "--device", type=str, default=None, help="Torch device, e.g. cpu or cuda."
    )
    return parser
...
def main():
    ...
    require_fields(payload, {"environment_config", "training"}, "experiment")
    ...
    elif training_values.get("device") == "auto":
        training_values["device"] = "cuda" if torch.cuda.is_available() else "cpu"
    if args.updates is not None:
        training_values["updates"] = args.updates
    config = QCMDPConfig.from_mapping(training_values)
    environment_path = resolve_relative_path(
        args.config_json, str(payload["environment_config"])
    )
    environment_config = load_json(environment_path)
    ...
    plot_training_rewards(
        training_history,
        str(output_dir / "training_convergence.png"),
